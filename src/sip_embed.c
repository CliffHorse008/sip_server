...
static void sip_embed_service_get_stream(sip_embed_service_t *service,
                                         streamer_t **streamer,
                                         unsigned int *generation)
{
    pthread_mutex_lock(&service->mutex);
    *streamer = service->streamer;
    *generation = service->stream_generation;
    pthread_mutex_unlock(&service->mutex);
}

static void sip_embed_service_set_stream(sip_embed_service_t *service, streamer_t *streamer)
{
    pthread_mutex_lock(&service->mutex);
...
static int sip_embed_service_build_answer(sip_embed_service_t *service,
...
static void sip_embed_on_signal(const sip_signal_event_t *event, void *user_data)
{
    sip_embed_service_t *service = (sip_embed_service_t *) user_data;

    if (event->type == SIP_SIGNAL_DIALOG_ESTABLISHED) {
        sip_embed_service_set_stream(service, event->streamer);
    } else if (event->type == SIP_SIGNAL_DIALOG_TERMINATED) {
        sip_embed_service_set_stream(service, NULL);
    }
...
static int sip_embed_on_invite(const sip_invite_event_t *event,
                               sip_invite_response_t *response,
                               void *user_data)
{
    sip_embed_service_t *service = (sip_embed_service_t *) user_data;
    ...
    return sip_embed_service_build_answer(service, event, response);
}

static void sip_embed_on_media(const streamer_rtp_packet_t *packet, void *user_data)
{
    sip_embed_service_t *service = (sip_embed_service_t *) user_data;
...
int sip_embed_service_run(sip_embed_service_t *service, volatile sig_atomic_t *stop_flag)
{
    ...
    handlers.on_signal = sip_embed_on_signal;
    handlers.on_invite = sip_embed_on_invite;
    handlers.on_media = sip_embed_on_media;
...
void sip_embed_service_get_stream_state(sip_embed_service_t *service, int *stream_active, unsigned int *generation)
{
    ...
    sip_embed_service_get_stream(service, &streamer, &current_generation);
    if (stream_active != NULL) {
                                       ...
                                       uint32_t rtp_timestamp)
{
    ...
    sip_embed_service_get_stream(service, &streamer, &generation);
    (void) generation;
    if (streamer == NULL) {
        return -1;
    }
